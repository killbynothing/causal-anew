...
> **北极星**：造一个持续存在的世界——角色会记忆、会判断、会主动行动，并彼此影响。机制服务于角色活着，不是目的本身。
> 原著是已完结长篇《存在的意义》。玩家自由文本进场；角色按知情与记忆自己说话。
...
→ 详见 [为什么这样做（含放弃过的路）](docs/design-philosophy.md)
...
python scripts/verify.py --quick                                 # 心跳自检（现 38 PASS / 0 FAIL）
